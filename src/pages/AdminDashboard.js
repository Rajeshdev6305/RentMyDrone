...
// import { FaEdit, FaTrash, FaEye, FaEyeSlash, FaPlus, FaSearch } from "react-icons/fa";
...
const AdminDashboard = ({ products: initialProducts, setProducts, currentAdmin }) => {
  const [formData, setFormData] = useState({
    name: "", model: "", pricePerHour: "", pricePerDay: "", pricePerMonth: "",
    controlRange: "", type: "", speed: "", weight: "", cameraQuality: "",
    payloadCapacity: "", sprayCapacity: "", batteryLife: "", usefulUses: "",
  ...
  }, [setProducts, location.state, initialProducts]);
  ...
  const handleAddProduct = (e) => {
    ...
    setTimeout(() => {
      if (editingProduct) {
        ...
        const newProduct = { id: Date.now(), ...formData, addedBy: currentAdmin };
        const updatedProducts = [...initialProducts, newProduct].sort((a, b) =>
          a.category.localeCompare(b.category)
        );
        setProducts(updatedProducts);
        Swal.fire("Success", "Product added successfully!", "success");
  ...
  const handleDeleteProduct = (productId) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        const updatedProducts = initialProducts.filter((product) => product.id !== productId);
        setProducts(updatedProducts);
        Swal.fire("Deleted!", "Product has been deleted.", "success");
      }
    });
  ...
  const filteredProducts = viewMyProducts
    ? initialProducts.filter((product) => product.addedBy === currentAdmin)
    : initialProducts;
