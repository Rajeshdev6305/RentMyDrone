...
const MainBody = ({ isLoggedIn, setCartItems, cartItems, products, searchTerm }) => {
  ...
  const handleAddToCart = (product) => {
    ...
    if (existingCartItem) {
      updatedCartItems = cartItems.map((item) =>
        item.id === product.id ? { ...item, quantity: item.quantity + 1, totalPrice: item.totalPrice + product.pricePerDay } : item
      );
    } else {
  ...
  return (
    <div className="">
      <h2 className="text-xl font-bold mb-4">Welcome to Drone Delivery Service</h2>
      <p className="text-sm mb-4">Explore our range of high-quality drones available for delivery.</p>
      ...
      {showCategories && (
        <div className="flex justify-center mb-4 space-x-2">
          <button
            ...
            className={`px-2 py-1 text-sm rounded ${selectedCategory === "All" ? "text-blue-600" : "hover:text-gray-700 transition"}`}
          >
            All
          </button>
          <button
            onClick={() => handleCategoryChange("Marriage")}
            className={`px-2 py-1 text-sm rounded ${selectedCategory === "Marriage" ? "text-blue-600" : "hover:text-gray-700 transition"}`}
          >
            Marriage
          </button>
          <button
            onClick={() => handleCategoryChange("Food Delivery")}
            className={`px-2 py-1 text-sm rounded ${selectedCategory === "Food Delivery" ? "text-blue-600" : "hover:text-gray-700 transition"}`}
          >
            Food Delivery
          </button>
          <button
            onClick={() => handleCategoryChange("Farming")}
            className={`px-2 py-1 text-sm rounded ${selectedCategory === "Farming" ? "text-blue-600" : "hover:text-gray-700 transition"}`}
          >
            Farming
          </button>
        </div>
