...
const App = () => {
  ...
  // Load default products and check for saved products in local storage
  useEffect(() => {
    const defaultProducts = [
      // Your default product objects go here
      {
        id: 1,
        name: "Marriage Drone X1",
        model: "X1",
  ...
  // Persist added products to local storage
  const handleAddProduct = (newProduct) => {
    const updatedProducts = [...products, newProduct];
    setProducts(updatedProducts);
    localStorage.setItem("products", JSON.stringify(updatedProducts.filter((p) => !p.isDefault)));
  };
            ...
            <Route
              ...
              element={
                isLoggedIn && userType === "admin" ? (
                  <AdminDashboard
                    products={products}
                    setProducts={handleAddProduct} // Updated to handle adding products
